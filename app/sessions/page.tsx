...
const TYPE_INITIALS: Record<string, string> = {
  yoga: "Y", pilates: "P", breath: "B", sound: "S", flow: "⟡", run: "R", hiit: "H",
};

const STRETCHY_FEE = 23;
function calcPrice(target: number, spots: number): number {
...
function getStatusDisplay(s: DBSession) {
  const holds = s.current_holds || 0;
  const confirmed = holds >= s.min_attendees;
  const currentPrice = confirmed
    ? calcPrice(s.host_target, holds)
    : calcPrice(s.host_target, s.min_attendees);

  if (s.state === "cancelled") return { label: "CANCELLED", color: "#888", price: null };
  if (s.state === "confirmed") return { label: "GOING AHEAD", color: "#4CAF82", price: currentPrice };
  if (confirmed) {
...
function SessionCard({ s }: { s: DBSession }) {
  ...
  const initial   = TYPE_INITIALS[s.movement_type] ?? "?";
    ...
    <Link href={`/sessions/${s.id}`} className="block group">
      ...
      >
        ...
        <div className="flex items-center gap-2 mb-3">
          <span className="font-mono text-xs font-bold px-2.5 py-1.5 rounded-pill" style={{ backgroundColor: typeColor, color: "#fff" }}>
            {typeLabel}
          </span>
        </div>

        {/* Title */}
        <h3 className="font-display font-bold text-ink leading-tight mb-1" style={{ fontSize: "22px" }}>
          {s.title}
        </h3>
          ...
          <div className="flex gap-1.5 flex-wrap">
            {Array.from({ length: s.min_attendees }).map((_, i) => (
              <div
                ...
                style={{ backgroundColor: i < holds ? typeColor : "rgba(26,26,26,0.12)" }}
              />
            ))}
            {holds > s.min_attendees && (
              <span className="font-mono text-xs font-bold" style={{ color: typeColor }}>+{holds - s.min_attendees}</span>
            )}
          </div>
        </div>
...
export default function SessionsPage() {
  ...
  useEffect(() => {
    fetch("/api/admin/sessions")
      .then((r) => r.json())
      .then((data: DBSession[]) => {
        // Only show open/confirmed sessions that haven't started yet
        const now = new Date();
        const upcoming = (data || []).filter(
          (s) => s.state !== "cancelled" && s.state !== "completed" && new Date(s.starts_at) > now
        );
    ...
    <main className="min-h-screen bg-cream">
      ...
      <section className="px-5 pb-3 max-w-lg mx-auto">
        ...
        <p className="text-muted text-sm mt-2">
          {loading ? "Loading sessions…" : `${sessions.length} session${sessions.length !== 1 ? "s" : ""} coming up.`}
        </p>
      </section>
      ...
      <section className="px-4 pb-10 max-w-lg mx-auto space-y-3">
        ...
        {!loading && sessions.length === 0 && (
          <div className="card text-center py-10">
            ...
            <p className="font-bold text-ink mb-2">No sessions yet</p>
            <p className="text-muted text-sm">Check back soon — we're getting things ready.</p>
          </div>
        )}
