...
// Hosted PNG (not inline SVG) — mobile mail clients (Gmail, some Apple Mail modes)
// strip inline <svg>, which is why the mark went missing on phones. Two colourways:
// cream/white mark on dark schemes, black on light. S_PATH is kept for reference.
void S_PATH;
function sLogo(color: string) {
  ...
  return `<img src="${APP_URL}/${file}" width="30" height="30" alt="Stretchy" style="display:inline-block;vertical-align:middle;border:0;" />`;
}
// ─── DESIGN SYSTEM ────────────────────────────────────────────────────────────
